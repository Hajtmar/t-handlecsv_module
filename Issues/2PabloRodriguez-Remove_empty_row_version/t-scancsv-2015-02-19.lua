-- Version 2015-02-19 
-- Changes : 19.2.2015 - add ignore empty lines in CSV files funcionality (inspired by Pablo Rodriguez) 
...
thirddata.scancsv = { -- Global variables
    ...
    gLineNo=0, --
    gNumRows=0, --
    gNumCols=0, --  number of columns (in first row) add 19.2.2015
    gNumEmptyRows=0, -- number of empty rows add 19.2.2015
    gFile2Scan=nil,--
    gIfEOF=false, --
    gIfNotEOF=true --
}
...
-- Complex update this function 19.2.2015: test empty lines (Pablo Rodrigues)
function thirddata.scancsv.getnumrows(file)
	local inpcsvfile=thirddata.scancsv.csvfilename(file)
	local numrows=0
	if not thirddata.scancsv.gUserRemoveEmptyRows then
		for line in io.lines(inpcsvfile) do numrows=numrows+1 end
	else -- thirddata.scancsv.gUserRemoveEmptyRows == true
	 local numemptyrows=0  
		for line in io.lines(inpcsvfile) do  
			 if line ~= nil then 
				line=string.gsub(line, "%s+", "")
				line=string.gsub(line, thirddata.scancsv.gUserCSVLeftDelimiter, "")
				line=string.gsub(line, thirddata.scancsv.gUserCSVRightDelimiter, "")
				line=string.gsub(line, thirddata.scancsv.gUserCSVSeparator, "")
				line=string.gsub(line, '""', "")
					if line ~= "" then numrows=numrows+1 
									  else numemptyrows=numemptyrows+1
					end 
			end --line ~= nil 		
		end -- for line in io.lines
	  thirddata.scancsv.gNumEmptyRows=numemptyrows	
	end -- if not thirddata.scancsv.gUserRemoveEmptyRows
  -- The header not to take
	...
	context.setgvalue("numrows",tostring(thirddata.scancsv.gNumRows)) -- \numrows macro defining
   context.setgvalue("numemptyrows",tostring(thirddata.scancsv.gNumEmptyRows)) -- \numemptyrows macro defining -- add 19.2.2015
return numrows
...
function thirddata.scancsv.putdataintocolumns(deftexmacros,index,content)
		 	  -- Experimental row:
				thirddata.scancsv.gCSVField[thirddata.scancsv.ar2xls(index)]=content -- Fill the column field from the act. line data
		 	  --thirddata.scancsv.gCSVField[index]=content -- not used
				thirddata.scancsv.gCSV[thirddata.scancsv.gCSV[index]]=content -- values are data from a CSV row of the table column
		  if deftexmacros then -- set the current contents of macro when deftexmacros = TRUE
          context.setgvalue('c'..thirddata.scancsv.ar2colnum(index), content) -- defining automatic TeX macros  \cA, \cB, atd. resp. \cI, \cII, ...  containing the contents of the line. Macros with names of the headers are updated automatically 			
          context.setgvalue('hc'..thirddata.scancsv.ar2colnum(index),'\\bch '..content..'\\ech') -- defining automatic TeX macros \hcA, \hcB, atd. resp. \hcI, \hcII, ...  containing 'hooked' contents of the line. Macros with names of the headers are updated automatically)
           -- Adding additional values for testing...
          context.setgvalue(thirddata.scancsv.tmn(thirddata.scancsv.gCSV[index]),content)
          context.setgvalue('h'..thirddata.scancsv.tmn(thirddata.scancsv.gCSV[index]),'\\bch '..content..'\\ech')
        end -- if deftexmacros
end -- thirddata.scancsv.putdataintocolumns(deftexmacros,index,content)


function thirddata.scancsv.testemptyrow(deftexmacros,csvline)
			local testemptyline=thirddata.scancsv.parsecsvdata(csvline) -- parse row
	 		local testemptylinestring=''
	  		for i = 1, #testemptyline do
	 		 testemptylinestring=testemptylinestring..testemptyline[i]
	  		end
	  		testemptylinestring=string.gsub(testemptylinestring, "%s+", "")
			if (testemptylinestring == '') then
				thirddata.scancsv.gNumEmptyRows=thirddata.scancsv.gNumEmptyRows+1
				for i = 1, thirddata.scancsv.gNumCols do 	-- create an associative array: the index column names
					thirddata.scancsv.putdataintocolumns(deftexmacros,i,'')
				end -- for
			else
 				for i = 1, #testemptyline do 	-- create an associative array: the index column names
	 				thirddata.scancsv.putdataintocolumns(deftexmacros,i,testemptyline[i])
				end -- for		
			end 
end -- thirddata.scancsv.testemptyrow(csvline)
...
-- the data in a CSV table are available via Lua variables gCSV.Name, gCSV.Surname, gCSV.BirthDate etc
	...
	then
	 	if not thirddata.scancsv.gUserRemoveEmptyRows then 
			thirddata.scancsv.testemptyrow(deftexmacros,csvline)
		else
			local testemptyline=thirddata.scancsv.parsecsvdata(csvline) -- parse row
	 		local testemptylinestring=''
	  		for i = 1, #testemptyline do
			...
			return(true) 	
			end -- if (testemptylinestring == '') then
   	   csvline=thirddata.scancsv.parsecsvdata(csvline) -- parse row 
		 	for i = 1, #csvline do 	-- create an associative array: the index column names
	 				thirddata.scancsv.putdataintocolumns(deftexmacros,i,csvline[i])
			end -- for
		end -- if not thirddata.scancsv.gUserRemoveEmptyRows then
...
Removing of nonepmty rows from CSV table:  {\bf ]]..tostring(thirddata.scancsv.gUserRemoveEmptyRows)..[[}\crlf
Number of nonepmty rows in the table:  {\bf ]]..'\\numrows'..[[}\crlf
Number of epmty rows in the table:  {\bf ]]..'\\numemptyrows'..[[}\crlf 
Macros supplying columns data in each row of table:  ]]..infomakra..[[\crlf
...
{\bf\backslash numrows} -- number of table nonempty lines ({\bf]]..'\\numrows'..[[})\crlf
{\bf\backslash numemptyrows} -- number of table empty lines ({\bf]]..'\\numemptyrows'..[[})\crlf
{\bf\backslash numline} -- number of the currently row (for use in print reports) \crlf
...
{\bf\backslash removeemptyrows} -- turn on ignoring blank rows in CSV file\crlf
{\bf\backslash leaveemptyrows} -- turn off ignoring blank rows in CSV file\crlf
...
-- begin add. 19.2.2015 --
function thirddata.scancsv.removeemptyrows()
 thirddata.scancsv.gUserRemoveEmptyRows=true 
end

interfaces.definecommand {
    name      = "removeemptyrows",
    macro     = thirddata.scancsv.removeemptyrows,
}

function thirddata.scancsv.leaveemptyrows()
 thirddata.scancsv.gUserRemoveEmptyRows=false
end

interfaces.definecommand {
    name      = "leaveemptyrows",
    macro     = thirddata.scancsv.leaveemptyrows,
}
