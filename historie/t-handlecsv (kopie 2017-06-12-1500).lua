...
\def\dodoopencsvfile#1{%
   ...
   \fi%
   % \nextline%-- change/add 2015-09-15, remove 2015-09-22
}%
...
\def\doloopif#1#2#3#4{%
    ...
    % \opencsvfile% commented 12.6.2018
    \resetnumline%
    % \nextrow% must be here - commented 12.6.2017
    \bfilehook%
...
%      \doloop{\ctxlua{if '#1'~='#3' then context('\\exitloop') else context('\\blinehook\\fourthparam\\elinehook\\ifEOF\\exitloop\\else\\nextrow\\fi') end}}%  thirddata.handlecsv.gCurrentLinePointer<thirddata.handlecsv.gNumRows
     ...
     whileeq=>{% {string}{whileeq}{string} ... % List all, while the test is  met. When first which not met then just quit. If first is not met, will list no record
	  %\doloop{\if\firstparam\thirdparam\blinehook\fourthparam\elinehook\nextrow\else\exitloop\fi\ifEOF\exitloop\fi}%
     \doloop{\ctxlua{if '#1'=='#3' then context('\\blinehook\\fourthparam\\elinehook\\nextrow') else context('\\exitloop\\ifEOF\\exitloop\\fi')end}}%
     },% end while % the comma , is very important here!!!
     whileneq=>{% {string}{whileeq}{string} ... % List all, while the test is not met. When first which met then just quit. If first is met, will list no record
	  %\doloop{\if\firstparam\thirdparam\blinehook\fourthparam\elinehook\nextrow\else\exitloop\fi\ifEOF\exitloop\fi}%
     \doloop{\ctxlua{if '#1'~='#3' then context('\\blinehook\\fourthparam\\elinehook\\nextrow') else context('\\exitloop\\ifEOF\\exitloop\\fi')end}}%
     },% end while % the comma , is very important here!!! 
...
%\def\doloopwhileeq#1#2#3{\doloopif{#1}{whileeq}{#2}{#3}}% \doloopwhile{\Trida}{3.A}{\tableaction}% List when the test is met, other just quit.
%\def\doloopwhileneq#1#2#3{\doloopif{#1}{whileneq}{#2}{#3}}% \doloopwhile{\Trida}{3.A}{\tableaction}% List when the test is not met, other just quit.
\letvalue{doloopwhile}=\doloopuntilneq
\letvalue{doloopwhileeq}=\doloopuntilneq
\letvalue{doloopwhileneq}=\doloopuntileq
...
% \doloopwhileeq{<value1>}{<value2>}{<\actionmacro>} % the same as \doloopwhile
% \doloopwhileneq{<value1>}{<value2>}{<\actionmacro>}
