...
-- open the table and load it into the global variable thirddata.handlecsv.gTableRows
-- if the option thirddata.handlecsv.gCSVHeader==true is enabled, then into glob variable thirddata.handlecsv.gColumnNames
-- sets the column names from the title, if not then sets XLS notation, ie. cA, cB, cC, ...
-- into global variables thirddata.handlecsv.gNumRows and  thirddata.handlecsv.gNumCols it saves the number of rows and columns of the table
...
--	 thirddata.handlecsv.setnumline(1)
...
--	 thirddata.handlecsv.g_gCurrentLinePointer[inpcsvfile]={}
	 thirddata.handlecsv.g_gTableRows[inpcsvfile]=thirddata.handlecsv.gTableRows -- array of contents of cells of CSV table -> g_gTableRows[csvfilename][row][column]
     thirddata.handlecsv.g_gNumLine[inpcsvfile]=thirddata.handlecsv.gNumLine -- global variable -      g_gNumLine['csvfilename']=0
	 thirddata.handlecsv.g_gNumRows[inpcsvfile]=thirddata.handlecsv.gNumRows -- global variable  - save number of rows of csv table: g_gNumRows['csvfilename']=0
     thirddata.handlecsv.g_gNumCols[inpcsvfile]=thirddata.handlecsv.gNumCols -- global variable  - save number of columns of csv table: g_gNumCols['csvfilename']=0
--	 thirddata.handlecsv.g_gCurrentLinePointer[inpcsvfile]=thirddata.handlecsv.gCurrentLinePointer, -- ie. CSV line number ie. number of the currently processed row: g_gCurrentLinePointer['csvfilename']=0
     thirddata.handlecsv.g_gColumnNames[inpcsvfile]=thirddata.handlecsv.gColumnNames -- array with column names (readings from header of CSV file): g_gColumnNames['csvfilename']
     thirddata.handlecsv.g_gColNames[inpcsvfile]=thirddata.handlecsv.gColNames -- associative array with column names for indexing use f.e. g_gColNames['csvfilename']['Firstname']=1, etc...
--	 thirddata.handlecsv.g_gTableRowsIndex[inpcsvfile]=thirddata.handlecsv.gTableRowsIndex -- array of flags of lines of CSV table -> g_gTableEmptyRowsIndex['csvfilename'][i]= true or false
...
function thirddata.handlecsv.readline(numberofline) --
 ...
 local returnpar=thirddata.handlecsv.readlineof(inpcsvfile,numberofline)
 return returnpar -- return true if numberofline is regular line, else return false
...
-- Main function. Read data, parse them etc.
function thirddata.handlecsv.puvreadline(numberofline) --
	local numberofline=numberofline
	local returnpar=false
	 if type(numberofline)~= 'number' then
	 	if numberofline==nil then
...
local xlsname=tostring(''..xlsname)
	...
	context(thirddata.handlecsv.getcellcontent(xlsname,thirddata.handlecsv.gCurrentLinePointer))
				 end
		    end
			})
...
function thirddata.handlecsv.assigncontentsof(line) -- put data into columns macros
...
-- after read of line this function put content of columns into specific TeX macros...
function thirddata.handlecsv.puvassigncontents(line) -- put data into columns macros
 	for i=1,	thirddata.handlecsv.gNumCols do
 		content='nil' -- 1.10.2015
 		if line ~= 'nil_line' then content = line[i] end
		local macroname=thirddata.handlecsv.tmn(thirddata.handlecsv.gColumnNames[i])
...
-- Read data from specific cell of the csv table
function thirddata.handlecsv.getcellcontent(csvfile,column,row)
	-- local returnparametr='nil'  -- 1.10.2015
	local returnparametr=''  -- 9.1.2016
	local column=column
	local row=row
...
function thirddata.handlecsv.addtonumlineof(inpcsvfile,numline)
  thirddata.handlecsv.g_NumLine[inpcsvfile]=thirddata.handlecsv.g_NumLine[inpcsvfile]+numline
end
...
--  context(thirddata.handlecsv.gNumLine) -- 23.6.2017
 return thirddata.handlecsv.gNumLine
end
...
function thirddata.handlecsv.numcols(csvfile)
 if csvfile == '' then
  	context(thirddata.handlecsv.gNumCols)
  else
    context(thirddata.handlecsv.g_gNumCols[csvfile])
 end
end
...
function thirddata.handlecsv.doloopfromto(from, to, action)
 ...
 local gnumrows=thirddata.handlecsv.gNumRows+0
 local from=from+0
 local to=to+0
 local step=1
 local docycle=true
...
function thirddata.handlecsv.doloopfornext(numberofrows, action)
 ...
 context[[\removeunwantedspaces]]
 local gnumrows=thirddata.handlecsv.gNumRows+0
 local from=thirddata.handlecsv.gCurrentLinePointer+0
 local to=thirddata.handlecsv.gCurrentLinePointer+numberofrows
 local step=1
 local docycle=true
  if from>to then
  		step=-1
...
\def\numcolsof[#1]{\ctxlua{context(thirddata.handlecsv.numcols('#1'))}}
...
\def\getcsvcell[#1](#2,#3){\ctxlua{context(thirddata.handlecsv.getcellcontent('#1',#2,#3))}}%
%\def\getcsvcell[#1,#2]{\ctxlua{context(thirddata.handlecsv.getcellcontent(#1,#2))}}%
%%%%%\def\getcsvcell[#1,#2]{\if!#2!\ctxlua{context(thirddata.handlecsv.getcellcontent(#1,thirddata.handlecsv.gCurrentLinePointer))}\else\ctxlua{context(thirddata.handlecsv.getcellcontent(#1,#2))}\fi}%
