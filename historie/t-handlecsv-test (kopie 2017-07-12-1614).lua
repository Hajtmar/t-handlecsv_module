...
function thirddata.handlecsv.notmarkemptylines()
 thirddata.handlecsv.gMarkingEmptyLines=false
   for row=1,thirddata.handlecsv.g_gNumRows[''] do
...
function thirddata.handlecsv.handlecsvfilename(filename) -- In the absence of the file name to use the global variable
   ...
   thirddata.handlecsv.gCurrentlyProcessedCSVFile = filename
   return filename
end
...
local isemptyline=false
	for column=1,thirddata.handlecsv.g_gNumCols[''] do
		linecontent=linecontent..thirddata.handlecsv.g_gTableRows[''][lineindex][column]
	end
	if linecontent=="" or linecontent==nil then
		isemptyline=true
		thirddata.handlecsv.gNumEmptyRows=thirddata.handlecsv.gNumEmptyRows+1
	end
	thirddata.handlecsv.g_gTableRowsIndex[''][lineindex]=isemptyline
 return isemptyline
end


function thirddata.handlecsv.emptylineevaluation(lineindex)
	if thirddata.handlecsv.g_gTableRowsIndex[''][lineindex] then
	...
	return thirddata.handlecsv.g_gTableRowsIndex[''][lineindex]
end


function thirddata.handlecsv.hooksevaluation()
	for i=1,#thirddata.handlecsv.g_gColumnNames[''] do
	 if not thirddata.handlecsv.texmacroisdefined('bch'..thirddata.handlecsv.g_gColumnNames[''][i]) then
	  context.setgvalue('bch'..thirddata.handlecsv.g_gColumnNames[''][i],'\\relax')
	 end
	 if not thirddata.handlecsv.texmacroisdefined('ech'..thirddata.handlecsv.g_gColumnNames[''][i]) then
	  context.setgvalue('ech'..thirddata.handlecsv.g_gColumnNames[''][i],'\\relax')
	 end
	end
end
...
function thirddata.handlecsv.opencsvfile(filetoscan) -- Open CSV tabule, inicialize variables
	-- open the table and load it into the global variable thirddata.handlecsv.g_gTableRows['']
	-- if the option thirddata.handlecsv.gCSVHeader==true is enabled, then into glob variable thirddata.handlecsv.g_gColumnNames['']
	-- sets the column names from the title, if not then sets XLS notation, ie. cA, cB, cC, ...
	-- into global variables thirddata.handlecsv.g_gNumRows[''] and  thirddata.handlecsv.g_gNumCols[''] it saves the number of rows and columns of the table
	...
	-- Additionally, they can defined ConTeXt macros  \csvfilename, \numrows a \numcols
	 ...
	 thirddata.handlecsv.g_gColNames['']={}
	 thirddata.handlecsv.g_gColumnNames['']={}
	 thirddata.handlecsv.g_gTableRowsIndex['']={}
	 thirddata.handlecsv.g_gTableRows['']={}
--	 thirddata.handlecsv.resetnumline() -- 22.6.2017
	...
	if thirddata.handlecsv.gCSVHeader then
	 thirddata.handlecsv.g_gTableRows[''], thirddata.handlecsv.g_gColumnNames[''] = mycsvsplitter(currentlyprocessedcsvfile,true)
	 inspect(thirddata.handlecsv.g_gTableRows[''])
	 inspect(thirddata.handlecsv.g_gColumnNames[''])
	else -- if thirddata.handlecsv.gCSVHeader
	  thirddata.handlecsv.g_gTableRows[''], thirddata.handlecsv.g_gColumnNames[''] = mycsvsplitter(currentlyprocessedcsvfile)
	  inspect(thirddata.handlecsv.g_gTableRows[''])
	  thirddata.handlecsv.g_gColumnNames['']={}
	  -- ad now set column names for withoutheader situation:
		for i=1,#thirddata.handlecsv.g_gTableRows[''][1] do
		 -- OK, but not used: thirddata.handlecsv.g_gColumnNames[''][i]=thirddata.handlecsv.tmn(thirddata.handlecsv.g_gTableRows[''][1][i])
		 thirddata.handlecsv.g_gColumnNames[''][i]=tostring(thirddata.handlecsv.ar2xls(i)) -- set XLS notation (fill array with XLS names of columns like 'cA', 'cB', etc.)
		end
	end -- if thirddata.handlecsv.gCSVHeader
	 	for i=1,#thirddata.handlecsv.g_gTableRows[''][1] do
	 	   thirddata.handlecsv.g_gColNames[''][tostring(thirddata.handlecsv.tmn(thirddata.handlecsv.g_gColumnNames[''][i]))] = i -- for indexing use (register names of macros ie 'Firstname' etc...)
			thirddata.handlecsv.g_gColNames[''][tostring(thirddata.handlecsv.g_gColumnNames[''][i])] = i -- for indexing use (register names of macros ie 'Firstname' etc...)
			thirddata.handlecsv.g_gColNames[''][tostring(thirddata.handlecsv.ar2xls(i))] = i -- for indexcolname macro (register names of macros ie 'A', 'B', etc...)
			thirddata.handlecsv.g_gColNames[''][tostring('c'..thirddata.handlecsv.ar2xls(i))] = i -- for indexcolname macro (register names of macros ie 'cA', 'cB', etc...)
		end
		local j=#thirddata.handlecsv.g_gTableRows[''][1]
		for i=1,#thirddata.handlecsv.g_gTableRows[''][1] do
		j=j+1
		thirddata.handlecsv.g_gColumnNames[''][j]=tostring('c'..thirddata.handlecsv.ar2xls(i)) -- set XLS notation (fill array with XLS names of columns like 'cA', 'cB', etc.)
		end
		if thirddata.handlecsv.gCSVHeader then
			for i=1,#thirddata.handlecsv.g_gTableRows[''][1] do
			j=j+1
			thirddata.handlecsv.g_gColumnNames[''][j]=tostring(thirddata.handlecsv.ar2xls(i)) -- set XLS notation (fill array with XLS names of columns like 'cA', 'cB', etc.)
			end
			for i=1,#thirddata.handlecsv.g_gTableRows[''][1] do
			j=j+1
			thirddata.handlecsv.g_gColumnNames[''][j]=tostring(thirddata.handlecsv.tmn(thirddata.handlecsv.g_gColumnNames[''][i])) -- maybe TeX incorect names of columns
			end
		end
 	 thirddata.handlecsv.g_gNumRows['']=#thirddata.handlecsv.g_gTableRows[''] -- Getting number of rows
  	 thirddata.handlecsv.g_gNumCols['']=#thirddata.handlecsv.g_gTableRows[''][1] -- Getting number of columns
...
--	 thirddata.handlecsv.g_gCurrentLinePointer[inpcsvfile]={}
	 thirddata.handlecsv.g_gTableRows[inpcsvfile]=thirddata.handlecsv.g_gTableRows[''] -- array of contents of cells of CSV table -> g_gTableRows[csvfilename][row][column]
     thirddata.handlecsv.g_gNumLine[inpcsvfile]=thirddata.handlecsv.g_gNumLine[''] -- global variable -      g_gNumLine['csvfilename']=0
	 thirddata.handlecsv.g_gNumRows[inpcsvfile]=thirddata.handlecsv.g_gNumRows[''] -- global variable  - save number of rows of csv table: g_gNumRows['csvfilename']=0
     thirddata.handlecsv.g_gNumCols[inpcsvfile]=thirddata.handlecsv.g_gNumCols[''] -- global variable  - save number of columns of csv table: g_gNumCols['csvfilename']=0
--	 thirddata.handlecsv.g_gCurrentLinePointer[inpcsvfile]=thirddata.handlecsv.g_gCurrentLinePointer[''], -- ie. CSV line number ie. number of the currently processed row: g_gCurrentLinePointer['csvfilename']=0
     thirddata.handlecsv.g_gColumnNames[inpcsvfile]=thirddata.handlecsv.g_gColumnNames[''] -- array with column names (readings from header of CSV file): g_gColumnNames['csvfilename']
     thirddata.handlecsv.g_gColNames[inpcsvfile]=thirddata.handlecsv.g_gColNames[''] -- associative array with column names for indexing use f.e. g_gColNames['csvfilename']['Firstname']=1, etc...
--	 thirddata.handlecsv.g_gTableRowsIndex[inpcsvfile]=thirddata.handlecsv.g_gTableRowsIndex[''] -- array of flags of lines of CSV table -> g_gTableEmptyRowsIndex['csvfilename'][i]= true or false

end -- of thirddata.handlecsv.opencsvfile(file)



function thirddata.handlecsv.JEOKALEPOKOUSIMSEOZMENUopencsvfile(filetoscan) -- Open CSV tabule, inicialize variables
	-- open the table and load it into the global variable thirddata.handlecsv.g_gTableRows['']
	-- if the option thirddata.handlecsv.gCSVHeader==true is enabled, then into glob variable thirddata.handlecsv.g_gColumnNames['']
	-- sets the column names from the title, if not then sets XLS notation, ie. cA, cB, cC, ...
	-- into global variables thirddata.handlecsv.g_gNumRows[''] and  thirddata.handlecsv.g_gNumCols[''] it saves the number of rows and columns of the table
...
local inpcsvfile=thirddata.handlecsv.handlecsvfilename()
	thirddata.handlecsv.readlineof(inpcsvfile,numberofline) --
end
...
function thirddata.handlecsv.readlineof(inpcsvfile,numberofline) --
    local inpcsvfile=thirddata.handlecsv.handlecsvfilename(inpcsvfile)
	local numberofline=numberofline
	local returnpar=false
	 if type(numberofline)~= 'number' then
	 	if numberofline==nil then
	 	 numberofline=thirddata.handlecsv.g_gCurrentLinePointer[inpcsvfile]
	 	 returnpar=true
	 	else numberofline = 0
		end -- if numberofline==nil
	 end --  if type(numberofline)
	 if (numberofline > 0 and numberofline <=thirddata.handlecsv.g_gNumRows[inpcsvfile]) then
	 	 -- thirddata.handlecsv.g_gNumLine[''] = thirddata.handlecsv.g_gNumLine[''] + 1
	 	 thirddata.handlecsv.addtonumline(1)
	  	 thirddata.handlecsv.g_gCurrentLinePointer[inpcsvfile]=numberofline
		 returnpar=true
		 thirddata.handlecsv.assigncontents(thirddata.handlecsv.g_gTableRows[inpcsvfile][numberofline])
		 context([[\global\EOFfalse\global\notEOFtrue%]])
	 else
		 thirddata.handlecsv.assigncontents('nil_line')
			if numberofline > thirddata.handlecsv.g_gNumRows[inpcsvfile] then
...
local xlsname=tostring(''..xlsname)
		interfaces.definecommand (docxlsname, {
		    ...
		    macro = function (opt_1)
		       if #opt_1>0 then
	context(thirddata.handlecsv.getcellcontent(xlsname,tonumber(opt_1)))
				 else
	context(thirddata.handlecsv.getcellcontent(xlsname,thirddata.handlecsv.g_gCurrentLinePointer['']))
				 end
		    end
			})
...
local inpcsvfile=thirddata.handlecsv.handlecsvfilename(inpcsvfile)
 	for i=1,thirddata.handlecsv.g_gNumCols[inpcsvfile] do
 		content='nil' -- 1.10.2015
 		if line ~= 'nil_line' then content = line[i] end
		local macroname=thirddata.handlecsv.tmn(inpcsvfile..thirddata.handlecsv.g_gColumnNames[inpcsvfile][i])
--		context(macroname)
		local purexlsname=thirddata.handlecsv.ar2colnum(i)
		local xlsname='c'..purexlsname
		local xlsfilename=thirddata.handlecsv.tmn(inpcsvfile)..'c'..purexlsname
...
local inpcsvfile=thirddata.handlecsv.handlecsvfilename()
thirddata.handlecsv.assigncontentsof(inpcsvfile,line)
end

-- after read of line this function put content of columns into specific TeX macros...
function thirddata.handlecsv.xassigncontents(line) -- put data into columns macros
local inpcsvfile=thirddata.handlecsv.handlecsvfilename()
 	for i=1,	thirddata.handlecsv.g_gNumCols[inpcsvfile] do
 		content='nil' -- 1.10.2015
 		if line ~= 'nil_line' then content = line[i] end
		local macroname=thirddata.handlecsv.tmn(thirddata.handlecsv.g_gColumnNames[inpcsvfile][i])
		local purexlsname=thirddata.handlecsv.ar2colnum(i)
		local xlsname='c'..purexlsname
		local hookxlsname='h'..xlsname
		local macroname=thirddata.handlecsv.tmn(macroname)
...
function thirddata.handlecsv.getcellcontentof(csvfile,column,row)
	...
	local csvfile=thirddata.handlecsv.handlecsvfilename(csvfile)
	local returnparametr=''  -- 9.1.2016
	local column=column
	local row=row
	if type(column)=='string' then
		local testcolumn=thirddata.handlecsv.g_gColNames[csvfile][column]
...
--		  		tex.sprint('*****')
	...
	if type(column)=='number' and type(row)=='number' then
		if row>0 and row <=thirddata.handlecsv.g_gNumRows[csvfile] and column>0 and column<=thirddata.handlecsv.g_gNumCols[csvfile] then
	 		returnparametr=thirddata.handlecsv.g_gTableRows[csvfile][row][column]
		elseif row==0 then
	 		returnparametr=thirddata.handlecsv.g_gColumnNames[csvfile][column]
	 	end
	end
...
function thirddata.handlecsv.getcellcontent(column,row)
 local csvfile=thirddata.handlecsv.handlecsvfilename()
...
function thirddata.handlecsv.nextline()
  if thirddata.handlecsv.g_gCurrentLinePointer[''] > thirddata.handlecsv.g_gNumRows[''] then
  	  thirddata.handlecsv.g_gCurrentLinePointer['']=thirddata.handlecsv.g_gNumRows['']
     context([[\global\EOFtrue%]])
     context([[\global\notEOFfalse%]])
  end
  thirddata.handlecsv.g_gCurrentLinePointer['']=thirddata.handlecsv.g_gCurrentLinePointer['']+1
end

function thirddata.handlecsv.nextlinex()
  if thirddata.handlecsv.g_gCurrentLinePointer[''] > thirddata.handlecsv.g_gNumRows[''] then
  	  thirddata.handlecsv.g_gCurrentLinePointer['']=thirddata.handlecsv.g_gNumRows['']
     context([[\global\EOFtrue%]])
     context([[\global\notEOFfalse%]])
...
function thirddata.handlecsv.setlinepointerof(csvfile,numberofline)
  local csvfile=thirddata.handlecsv.handlecsvfilename(csvfile)
  thirddata.handlecsv.g_gCurrentLinePointer[csvfile]=numberofline
...
function thirddata.handlecsv.linepointer()
  context(thirddata.handlecsv.g_gCurrentLinePointer[''])
end


function thirddata.handlecsv.setnumline(numline)
  thirddata.handlecsv.g_gNumLine['']=numline
end
...
function thirddata.handlecsv.addtonumline(numline)
  thirddata.handlecsv.g_gNumLine['']=thirddata.handlecsv.g_gNumLine['']+numline
end

function thirddata.handlecsv.numline()
--  context(thirddata.handlecsv.g_gNumLine['']) -- 23.6.2017
 return thirddata.handlecsv.g_gNumLine['']
end
...
function thirddata.handlecsv.numrows(csvfile)
  context(thirddata.handlecsv.g_gNumRows[csvfile])
end

function thirddata.handlecsv.xnumrows(csvfile)
 local csvfile=thirddata.handlecsv.handlecsvfilename(csvfile)
 if csvfile == '' then
  	context(thirddata.handlecsv.g_gNumRows[''])
  else
    context(thirddata.handlecsv.g_gNumRows[csvfile])
 end
end
...
function thirddata.handlecsv.numnotemptyrows()
  context(thirddata.handlecsv.g_gNumRows['']-thirddata.handlecsv.gNumEmptyRows)
end


function thirddata.handlecsv.numcols()
  context(thirddata.handlecsv.g_gNumCols[''])
end
...
function thirddata.handlecsv.doloopfromto(from, to, action)
 ...
 local gnumrows=thirddata.handlecsv.g_gNumRows['']+0
 local from=from+0
 local to=to+0
 local step=1
 local docycle=true
...
function thirddata.handlecsv.doloopfornext(numberofrows, action)
 ...
 context[[\removeunwantedspaces]]
 local gnumrows=thirddata.handlecsv.g_gNumRows['']+0
 local from=thirddata.handlecsv.g_gCurrentLinePointer['']+0
 local to=thirddata.handlecsv.g_gCurrentLinePointer['']+numberofrows
 local step=1
 local docycle=true
  if from>to then
  		step=-1
...
\def\numrowsof[#1]{\ctxlua{context(thirddata.handlecsv.numrows('#1'))}}
...
%%%%%\def\getcsvcell[#1,#2]{\if!#2!\ctxlua{context(thirddata.handlecsv.getcellcontent(#1,thirddata.handlecsv.g_gCurrentLinePointer['']))}\else\ctxlua{context(thirddata.handlecsv.getcellcontent(#1,#2))}\fi}%
...
\def\colname[#1]{\ctxlua{context(thirddata.handlecsv.g_gColumnNames[''][#1])}}%

% Get index (ie serrial number) of strings columns names (own name or XLS name)
\def\indexcolname[#1]{\ctxlua{context(thirddata.handlecsv.g_gColNames[''][#1])}}%
...
\def\texcolname[#1]{\ctxlua{context(thirddata.handlecsv.tmn(thirddata.handlecsv.g_gColumnNames[''][#1]))}}%
...
\def\dodoreadline#1{\iffirstargument\ctxlua{thirddata.handlecsv.readline(#1)}\else\ctxlua{thirddata.handlecsv.readline(thirddata.handlecsv.g_gCurrentLinePointer[''])}\fi}%
