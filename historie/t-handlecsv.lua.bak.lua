...
function thirddata.handlecsv.doloopfromto(from, to, action)
 context[[\opencsvfile]]
-- context[[\resetnumline]] -- commented 22.6.2017
 context[[\bfilehook]]
...
\def\doloopif#1#2#3#4{%
    ...
    \processaction[\paroperator][%
     ...
     untileq=>{% {substring}{untileq}{string} ... % List all, until the test is not met - then just quit. Repeat until satisfied. If it is not never met, will list all record
      \doloop{\ctxlua{if '#1'=='#3' then context('\\exitloop') else context('\\ifEOF\\exitloop\\else\\blinehook\\fourthparam\\elinehook\\nextrow\\fi') end}}%  thirddata.handlecsv.gCurrentLinePointer<thirddata.handlecsv.gNumRows 
     },% end untileq % the comma , is very important here!!!     
     untilneq=>{% {substring}{untilneq}{string} ... % List all, until the test is not met - then just quit. Repeat until satisfied. If it is not never met, will list all record
      \doloop{\ctxlua{if '#1'~='#3' then context('\\exitloop') else context('\\ifEOF\\exitloop\\else\\blinehook\\fourthparam\\elinehook\\nextrow\\fi') end}}%  thirddata.handlecsv.gCurrentLinePointer<thirddata.handlecsv.gNumRows 
...
\def\doloopuntileq#1#2#3{\doloopif{#1}{untileq}{#2}{#3}}% \doloopuntileq{\Trida}{3.A}{\tableaction}% List all, until the test is not met - then just quit. Repeat until satisfied. If it is not never met, will list all records.
\def\doloopuntilneq#1#2#3{\doloopif{#1}{untilneq}{#2}{#3}}% \doloopuntilneq{\Trida}{3.A}{\tableaction}% List all, until the test is met - then just quit. Repeat until satisfied. If it is always met, will list no records.
\letvalue{doloopwhile}=\doloopuntilneq% equivalent functions % \doloopwhile{\Trida}{3.A}{\tableaction} % list when the test is met, other just quit. FOR COMPATIBILITY ONLY
\letvalue{doloopwhileeq}=\doloopuntilneq% equivalent functions, \doloopwhileeq{\Trida}{3.A}{\tableaction} % list when the test is met, other just quit.
\letvalue{doloopwhileneq}=\doloopuntileq% equivalent functions, \doloopwhileneq{\Trida}{3.A}{\tableaction} % list when the test is not met, other just quit.
...
% \doloopuntileq{<value1>}{<value2>}{<\actionmacro>}
% \doloopuntilneq{<value1>}{<value2>}{<\actionmacro>}
% \doloopwhile{<value1>}{<value2>}{<\actionmacro>} %  the same as \doloopuntilneq
% \doloopwhileeq{<value1>}{<value2>}{<\actionmacro>} % the same as \doloopwhile and the same as \doloopuntilneq
% \doloopwhileneq{<value1>}{<value2>}{<\actionmacro>} % the same as \doloopuntileq
% \filelineaction, \filelineaction{<filename>}
% \doloopfornext{<+/-numberofrows>}{<\actionmacro>} % use \setlinepointer,  \resetlinepointer to set line pointer. Opening of CSV file automatically reset line pointer.
