...
--??  	 	 context.setgvalue(hookxlsname,'\\bch'..xlsname..'\\'..xlsname..' \\ech'..xlsname) -- defining automatic TeX macros \hcA, \hcB, atd. resp. \hcI, \hcII, ...  containing 'hooked' contents of the line. Macros with names of the headers are updated automatically)
--	unused	 context.setgvalue(macroname,content) -- defining automatic TeX macros  \cName, \cDate, etc. (names gets from header), containing the contents of the line. Macros with names of the headers are updated automatically
...
--???       context.setgvalue(hookmacroname,'\\bch'..macroname..'\\'..xlsname..' \\ech'..macroname) -- defining automatic TeX macros \hName, \hDate, etc. (names gets from header), containing 'hooked' contents of the line. Macros with names of the headers are updated automatically)
  	end
...
\def\colname[#1]{\directlua{context(thirddata.handlecsv.gColumnNames[#1])}}%

% Get index (ie serrial number) of strings columns names (own name or XLS name) 
\def\indexcolname[#1]{\directlua{context(thirddata.handlecsv.gColNames[#1])}}%


% Get (alternative) XLS column name (of n-th column)
\def\xlscolname[#1]{\directlua{context(thirddata.handlecsv.ar2colnum(#1))}}%
...
\def\numberxlscolname[#1]{\directlua{context(thirddata.handlecsv.xls2ar(#1))}}%
...
% \colname[numberofcolumn]
% \indexcolname[<'columnname' or 'xlsname'>]
% \xlscolname[<numberofcolumn>]
