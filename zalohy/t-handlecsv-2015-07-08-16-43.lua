...
function thirddata.handlecsv.createxlscommand(macroname)
local macroname=macroname
		interfaces.definecommand ('do'..macroname, {
		    arguments = { { "option", "string" }  },
		    macro = function (opt_1)
		       if #opt_1>0 then 
				  context('\\csvcell["'..tostring(macroname)..'",'..opt_1..']')
				 else
				  context('\\csvcell["'..tostring(macroname)..'",\\linepointer]')
				 end 
		    end
			})
		interfaces.definecommand (macroname, {
		    macro = function ()
		      context.dosingleempty()
		      context['do'..macroname]()
		    end
			})
end 
...
function thirddata.handlecsv.assigncontents(line) -- put data into columns macros
 	...
 	for i=1,	thirddata.handlecsv.gNumCols do
	   ...
	   context.setgvalue(xlsname, content) -- defining automatic TeX macros  \cA, \cB, atd. resp. \cI, \cII, ...  containing the contents of the line. Macros with names of the headers are updated automatically 			
		thirddata.handlecsv.createxlscommand('c'..xlsname)
		context.setgvalue(macroname,'\\'..xlsname) -- correction 3.7.2015  ie  for example \let\Name\cA 
		-- context.setgvalue('c'..macroname,'\\c'..xlsname) -- correction 3.7.2015  ie  for example \let\Name\cA
