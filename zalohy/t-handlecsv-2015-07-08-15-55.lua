		...
		interfaces.definecommand ('do'..xlsname, {
		    ...
		    macro = function (opt_1)
		       if #opt_1>0 then 
				  ...
				  -- context(thirddata.handlecsv.gTableRows[tonumber(opt_1)][thirddata.handlecsv.xls2ar(tostring(xlsname))])
				  -- context.setvalue(xlsname, content) -- defining automatic TeX macros  \cA, \cB, atd. resp. \cI, \cII, ...  containing the contents of the line. Macros with names of the headers are updated automatically 
				 else 
				  --context('\\csvcell["'..xlsname..'",\\linepointer]')
				  context('\\expanded{\\csvcell["'..xlsname..'",'..thirddata.handlecsv.gCurrentLinePointer..']}')
				--   context(thirddata.handlecsv.gTableRows[thirddata.handlecsv.gCurrentLinePointer][thirddata.handlecsv.xls2ar(tostring(xlsname))])
				--	context.setvalue(xlsname, content) -- defining automatic TeX macros  \cA, \cB, atd. resp. \cI, \cII, ...  containing the contents of the line. Macros with names of the headers are updated automatically				   
				 end 
		    end
			})
...
% \numcols
% \csvfilename
% \xcsvreport,  \xcsvreport{<filename>}
% \xprintline
% \xprintall
% \thenumexp{<expression>}
